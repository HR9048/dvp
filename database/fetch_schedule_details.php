...
if ($location == "Division") {
    ...
    $condition2 = "sm.division_id = '$division_id'";
    ...
    $condition2 = "sm.depot_id = '$depot_id' and sm.division_id = '$division_id'";
...
if ($type === 'difference') {
    ...
    $operatedSchedules = [];
    $queryOperated = "SELECT sch_no, depot_id FROM sch_veh_out WHERE departed_date = '$today' AND ($condition)";
...
LEFT JOIN location l ON sm.depot_id = l.depot_id  -- Joining with the location table
WHERE 
    sm.status = 1
    AND $condition2
    AND sm.sch_dep_time <= '$curr_time'
ORDER BY 
    l.depot_id, sm.sch_dep_time ASC;
";
...
JOIN location l ON sm.depot_id = l.depot_id  -- Joining with location table
WHERE 
