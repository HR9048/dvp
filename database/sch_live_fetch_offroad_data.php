...
echo '<h2 style="text-align:center;">Depot-wise Off-Road Summary</h2>';
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($prev_division !== null && $prev_division !== $division) {
        ...
        echo "<tr style='background-color: #d4edda; font-weight: bold;'>
            <td colspan='2' onclick='fetchoffroadDetails(\"{$prev_division}\", \"{$prev_division}\", \"Division\")'>{$prev_division} Total</td>
            <td>{$division_totals[$prev_division]['TOTAL_BUS']}</td>
            <td>{$division_totals[$prev_division]['DEPOT_COUNT']}</td>
            <td>{$division_totals[$prev_division]['DWS']}</td>
    ...
    echo "<tr>
        <td>{$serial_number}</td>
        <td onclick='fetchoffroadDetails(\"{$row['division_id']}\", \"{$row['depot_name']}\", \"Depot\")'>{$row['DEPOT']}</td>
        <td>{$row['TOTAL_BUS']}</td>
