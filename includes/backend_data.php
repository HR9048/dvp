...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'fetch_schedule_details') {
    $schedule_key_no = $_POST['schedule_key_no'];
    $division_id = $_SESSION['DIVISION_ID'];
    $depot_id = $_SESSION['DEPOT_ID'];

    $stmt = $db->prepare("SELECT * FROM schedule_master WHERE sch_key_no = ? AND division_id = ? AND depot_id = ? and status != '0'");
    $stmt->bind_param("sss", $schedule_key_no, $division_id, $depot_id);
    $stmt->execute();
    $result = $stmt->get_result();
