...
?>
    ...
    $('#depot').append('<option value="All">All</option>');

    // Then append fetched depot options
    $('#depot').append(data);
        ...
        $(document).ready(function() {
            $('#scheduleForm').on('submit', function(e) {
                ...
                if (month < <?php echo 8; ?> && year < 2026) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Invalid Date',
                        text: 'Please select a month and year between Aug-2025 and <?php echo date('M-Y'); ?>. Reports available only for this range.',
                        confirmButtonText: 'OK'
                    });
                    return; // Exit the function if validation fails
                }
