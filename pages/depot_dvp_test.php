...
<div class="container">
    ...
    <div class="card" id="step2" style="display:none;">
        <form id="breakdownFormData">
            <h5 class="text-warning mb-4"><i class="bi bi-tools"></i> Breakdown Details</h5>
            <div class="mb-3">
                <label class="form-label">Breakdown Date</label>
                <input type="date" class="form-control" name="bd_date" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label class="form-label">KM After Docking</label>
                <input type="number" class="form-control" name="km_after_docking" required>
            </div>
    ...
    <div class="card" id="step3" style="display:none;">
        <form id="dvpFormData">
            <h5 class="text-success mb-4"><i class="bi bi-clipboard2-check-fill"></i> DVP Details</h5>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="program_type" required>
            </div>
            <div class="mb-3">
                <label class="form-label">KM Completed</label>
                <input type="number" class="form-control" name="program_completed_km" required>
            </div>
            <div class="d-flex justify-content-between">
...
$(function(){
    ...
    // Navigation buttons
    $("#back2").click(() => { $("#step2").hide(); $("#step1").fadeIn(); updateStepIndicator(1); });
    $("#back3").click(() => { $("#step3").hide(); $("#step2").fadeIn(); updateStepIndicator(2); });

    // Step 1 Save
    $("#save1").click(function(){
            // Save data locally, don't submit
            formData.offroad = $("#offroadFormData").serializeArray();
            Swal.fire("Saved!", "Offroad details stored. Please add Breakdown if applicable.", "success");
            $("#step1").hide(); $("#step2").fadeIn(); updateStepIndicator(2);
        
    });

    // Step 2 Save
    $("#save2").click(function(){
        if ($("#breakdownFormData")[0].checkValidity()) {
            // Save data locally, don't submit
            formData.breakdown = $("#breakdownFormData").serializeArray();
            Swal.fire("Saved!", "Breakdown details stored locally.", "success");
            $("#step2").hide(); $("#step3").fadeIn(); updateStepIndicator(3);
        } else {
            $("#breakdownFormData")[0].reportValidity();
        }
    });

    // Step 3 Save & Finish
    $("#save3").click(function(){
        if ($("#dvpFormData")[0].checkValidity()) {
            // Save final form data
            formData.dvp = $("#dvpFormData").serializeArray();

            // Convert all form data to a single object
            let finalData = {};
            $.each(formData.offroad || [], (i, field) => finalData[field.name] = field.value);
            $.each(formData.breakdown || [], (i, field) => finalData[field.name] = field.value);
            $.each(formData.dvp || [], (i, field) => finalData[field.name] = field.value);

            // Now only submit here
            $.post("save_dvp.php", finalData, function(res){
                Swal.fire("All Done!", res, "success");
                $("form").trigger("reset");
                $("#step3").hide(); $("#step1").fadeIn(); updateStepIndicator(1);
                formData = {}; // Clear stored data
            });

        } else {
            $("#dvpFormData")[0].reportValidity();
        }
    });
