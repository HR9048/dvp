...
function getMaxAllowedDrivers($serviceClassName, $serviceTypeName, $singleCrewOperation)
{
    ...
    switch ($serviceClassName) {
        ...
        case '3':
            ...
            break;
        case '4':
            $maxAllowedDrivers = $singleCrewOperation === 'yes' ? 3 : 3;
            break;
        case '5':
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($numberOfDrivers > $maxAllowedDrivers) {
        ...
        if (!$validDrivers) {
            ...
            $updateScheduleSql = "UPDATE schedule_master SET 
                single_crew = ?
                WHERE id = ?";
            ...
            if ($stmt->execute()) {
                ...
                for ($i = 1; $i <= $numberOfDrivers; $i++) {
                    ...
                    $updateDriverSql = "UPDATE schedule_master SET 
                        ...
                        driver_name_" . $i . " = ?
                        WHERE id = ?";
                    // Prepare and bind parameters for insert or update query
                ...
                for ($i = 1; $i <= $numberOfConductor; $i++) {
                    ...
                    $updateConductorSql = "UPDATE schedule_master SET 
                        conductor_token_" . $i . " = ?,
                        conductor_pf_" . $i . " = ?,
                        conductor_name_" . $i . " = ?
                        WHERE id = ?";
