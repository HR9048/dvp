...
if ($_SESSION['TYPE'] == 'DEPOT' && ($_SESSION['JOB_TITLE'] == 'Mech' || $_SESSION['JOB_TITLE'] == 'DM')) {
    ...
    $bus_query = "SELECT br.bus_number, br.make, br.emission_norms, br.bus_progressive_km_31032025, br.model_type, bs.bus_type, bs.bus_category, br.bus_sub_category FROM bus_registration br left join bus_seat_category bs on bs.bus_sub_category= br.bus_sub_category WHERE br.depot_name = $depot_id AND br.division_name = $division_id and br.doc <= '2025-09-30' ORDER BY br.model_type ASC, br.bus_number ASC";
    ...
    while ($bus = mysqli_fetch_assoc($bus_result)) {
        ...
        $program_query = "SELECT id, program_type, program_completed_km, updated_at FROM program_data WHERE bus_number = '$bus_number'";
        $program_result = mysqli_query($db, $program_query);

        $program_data = [];
        while ($row = mysqli_fetch_assoc($program_result)) {
            $program_data[$row['program_type']] = [
                'id' => $row['id'],
                'program_completed_km' => $row['program_completed_km'],
...
?>
                ...
                <tbody>
                    ...
                    foreach ($all_rows as $row_data) {
                        ...
                        foreach ($visible_program_types as $type) {
                            ...
                            // Handle Air Suspension case first
                            ...
                            // If program not applicable for this bus
                            if (!$prog_val_row || is_null($prog_val_row[$type])) {
                                echo "<td>NA</td>";
                                continue;
                            }

                            // If program data exists
                            if (isset($program_data[$type])) {
                                $prog = $program_data[$type];
                                $id = $prog['id'];
                                $km = htmlspecialchars($prog['program_completed_km']);
                                $updated_at = $prog['updated_at'];

                                if (!empty($updated_at)) {
                                    // Show only text (no input)
                                    $cell = "<td>" . $km . "</td>";
                                } else {
                                    // Editable input if updated_at is null
                                    $cell = "<td>
                        <input type='number' class='km-input' 
                               name='{$type}_km[{$bus_number}]' 
                               value='{$km}' 
                               data-id='{$id}' 
                               placeholder='KM'>
                     </td>";
                    ...
                    <input type='number' class='km-input' 
                        ...
                        echo "<td><button class='update-btn' onclick=\"updateProgramData('$bus_number', document.getElementById('row-$bus_number'))\">Update</button></td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
    ...
    <script>
        function updateProgramData(busNumber, rowElement) {
            ...
            // Collect all editable inputs in the row (only those that exist)
            $(rowElement).find("input[type='number']").each(function() {
                const nameAttr = $(this).attr('name');
                const value = $(this).val()?.trim();
                const id = $(this).data('id') || null; // may be null if new entry

                // Example name format: 'fuel_filter_change_km[KA01AA0001]'
                const match = nameAttr?.match(/^(.+?)_km\[(.+?)\]$/);
                if (match) {
                    const programType = match[1]; // e.g., 'fuel_filter_change'
            ...
            // Validation: ensure at least one KM entered
            if (data.programs.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'No Data',
                    text: 'No editable KM fields found or filled in for this bus.'
                });
                return;
            }
