...
.custom-size {
    font-size: 0.6rem; /* Adjust for smaller devices */
}

/* Medium screens (tablets) */
@media (min-width: 768px) {
    .custom-size {
        font-size: 0.5rem; /* Slightly larger font size */
    }
}
...
@media (min-width: 1200px) {
    ...
    <div class="container" style="padding:2px">
        ...
        <nav>
            <div class="nav nav-tabs justify-content-center" id="nav-tab" role="tablist">
                <button class="nav-link active custom-size" id="nav-bus-tab" data-bs-toggle="tab" data-bs-target="#nav-bus"
                    type="button" role="tab" aria-controls="nav-bus" aria-selected="false">ವಾಹನ ಬದಲಾವಣೆ</button>
                <button class="nav-link custom-size" id="nav-crew-tab" data-bs-toggle="tab" data-bs-target="#nav-crew"
                    type="button" role="tab" aria-controls="nav-profile" aria-selected="false">ಸಿಬ್ಬಂದಿ ಬದಲಾವಣೆ</button>
            </div>
        </nav>
        <div class="tab-content">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control select2" id="sch_no_change_bus" name="sch_no_change_bus"
            ...
            <div class="tab-pane fade" id="nav-crew" role="tabpanel" aria-labelledby="nav-crew-tab">
                ...
                <input class="form-control" type="hidden" id="formtype_crew" value="crew" name="formtype_crew">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                <label for="sch_no_change_crew">ಅನುಸೂಚಿ ಕೀ ಸಂಖ್ಯೆ</label>
                                <select class="form-control select2" id="sch_no_change_crew" name="sch_no_change_crew"
                                    required style="min-width: 100px;">
                                    <option value="">ಅನುಸೂಚಿ ಕೀ ಸಂಖ್ಯೆಯನ್ನು ಆಯ್ಕೆಮಾಡಿ</option>
                                </select>
                            </div>
                ...
                function fetchScheduleOptions() {
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $('#sch_no_change_bus, #sch_no_change_crew').empty().append('<option value="">ಅನುಸೂಚಿ ಕೀ ಸಂಖ್ಯೆಯನ್ನು ಆಯ್ಕೆಮಾಡಿ</option>');
                            $.each(options, function (index, value) {
                                $('#sch_no_change_bus, #sch_no_change_crew').append('<option value="' + value + '">' + value + '</option>');
                            });
                        },
                        error: function (xhr, status, error) {
                ...
                function fetchBusScheduleDetails() {
                    ...
                    if (scheduleNo && outDate) {
                        $.ajax({
                            ...
                            type: 'POST',
                            data: { scheduleNo: scheduleNo, outDate: outDate, formType: formType},
                            success: function (response) {
