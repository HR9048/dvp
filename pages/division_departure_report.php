...
if ($_SESSION['TYPE'] == 'DIVISION' && $_SESSION['JOB_TITLE'] == 'ASO(Stat)') {
    ...
    <div class="container1">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['selected_date'])) {
            ...
            $location_query = "SELECT division_id, depot_id, division, depot FROM location WHERE division_id= {$_SESSION['DIVISION_ID']}";
...
WHERE sm.division_id = ? 
  ...
  AND (sai.inact_to IS NOT NULL OR sai.sch_key_no IS NULL);";
            ...
            COUNT(svo.sch_no) AS total_departures,
            COUNT(CASE WHEN svo.arr_date = ? THEN 1 END) AS total_arrival
        ...
        WHERE svo.division_id = ? 
          ...
          AND svo.departed_date = ?";
                ...
                $stmt2->bind_param('ssis', $selected_date, $division_id, $depot_id, $selected_date);
                $stmt2->execute();
                $result2 = $stmt2->get_result();
                $row2 = $result2->fetch_assoc();
                $total_departures = $row2['total_departures'];
                $total_arrival = $row2['total_arrival'];
