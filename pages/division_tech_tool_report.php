...
?>
    ...
    <form id="scheduleForm">
        ...
        <select id="depot" name="depot" required>
            ...
            <?php
            $depot_query = "SELECT depot_id, DEPOT FROM location WHERE division_id = '$division_id' ORDER BY depot_id ASC";
            $depot_result = mysqli_query($db, $depot_query);
            while ($depot_row = mysqli_fetch_assoc($depot_result)) {
                echo '<option value="' . $depot_row['depot_id'] . '">' . $depot_row['DEPOT'] . '</option>';
            }
            ?>
        </select>
